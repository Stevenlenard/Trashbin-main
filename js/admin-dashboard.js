...
let bins = [
  { id: "BIN-001", location: "Main Street - Corner Store", type: "General", status: "full", lastEmptied: "2 hours ago", capacity: "95%", assignedTo: "John Doe" },
  { id: "BIN-002", location: "Park Avenue - Central Park", type: "Recyclable", status: "empty", lastEmptied: "30 mins ago", capacity: "10%", assignedTo: "Jane Smith" },
  { id: "BIN-003", location: "Downtown - Market Square", type: "Organic", status: "needs_attention", lastEmptied: "5 hours ago", capacity: "80%", assignedTo: "Bob Johnson" },
];
...
let notifications = [
  { id: 1, type: "critical", message: "Bin BIN-001 is FULL - Immediate action required", time: "Just now", read: false },
  { id: 2, type: "warning", message: "Bin BIN-003 capacity at 80% - Schedule emptying soon", time: "5 mins ago", read: false },
...
function loadJanitorsTable() {
  ...
  janitors.forEach((j) => {
    const statusBadge = j.status === "active" ? '<span class="badge badge-success">Active</span>' : '<span class="badge badge-danger">Inactive</span>';
    const row = document.createElement("tr");
    row.innerHTML = `
      <td>${j.name}</td>
      <td>${j.email}</td>
...
function loadNotificationsTable() {
  ...
  notifications.forEach((notif) => {
    ...
    const statusBadge = notif.read ? '<span class="badge badge-info">Read</span>' : '<span class="badge badge-danger">Unread</span>';
    const row = document.createElement("tr");
    row.innerHTML = `
      <td>${notif.time}</td>
...
window.openEditJanitorModal = function(janitorId) {
...
window.saveJanitorEdit = function() {
  const janitorId = parseInt(document.getElementById("editJanitorId").value);
  const index = janitors.findIndex((j) => j.id === janitorId);
  ...
  janitors[index].bins = parseInt(document.getElementById("editJanitorBins").value);
...
window.viewReport = function(reportId) {
  ...
  // In real implementation, this would open the report in a new window or modal
};
...
window.saveNewJanitor = function() {
  ...
  const bins = parseInt(document.getElementById("newJanitorBins").value);
...
window.filterChart = function(period, chartType) {
